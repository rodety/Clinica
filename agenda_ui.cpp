    ...
    ui(new Ui::agenda_ui)
{
    ...
    ui->tablaView_Agend->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->tableWidget_Alert->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ui->tableWidget_Alert_Personales->setEditTriggers(QAbstractItemView::NoEditTriggers);
    ...
    ui->tablaView_Agend->setColumnWidth(0,100);
    ...
    ui->tablaView_Agend->setColumnWidth(5,0);
...
void agenda_ui::on_pushButton_newTask_clicked()
{
    ...
    NEW_TASK_FORM->setParentWindow(parentWindow);
    //INSERTANDO CODIGO PARA VERIFICAR SI CAMBIO EL WIDGET
    NEW_TASK_FORM->setcodeWindow(parentWindow->getcodeWindow());    
    NEW_TASK_FORM->enableDate(false);
    NEW_TASK_FORM->setTitle("Nueva Tarea");
    NEW_TASK_FORM->show();
    connect(NEW_TASK_FORM,SIGNAL(update_list()),this,SLOT(update_table()));
}
...
void agenda_ui::on_pushButton_editTask_clicked()
{
    ...
    EDIT_TASK_FORM->setModoUso(1);
    EDIT_TASK_FORM->setParentWindow(parentWindow);
    EDIT_TASK_FORM->setcodeWindow(parentWindow->getcodeWindow());
    EDIT_TASK_FORM->show();
    connect(EDIT_TASK_FORM,SIGNAL(update_list()),this,SLOT(update_table()));
...
void agenda_ui::on_calendarWidget_clicked(const QDate &date)
{
    ...
    ui->dateEdit_Alerta->setDate(date);
    ui->dateEdit_Alerta_Personales->setDate(date);
}
...
{
    agenda_list_documento agenda;
    agenda.setCompany("PRAXIS MEDICAL SAC");
    agenda.setDate(ui->dateEdit_Agenda->text());
    QVector <QString> employes;
    QVector <QString> dniEmployes;
    int cant = ui->comboBox_Names->count();
    for(int i=0;i<cant;i++)
    {
        employes.push_back(ui->comboBox_Names->itemText(i));
        dniEmployes.push_back(ui->comboBox_dni->itemText(i));
    }
    agenda.setListEmploye(employes);
    agenda.setListDniEmploye(dniEmployes);

    agenda.generateLatex();
}

void agenda_ui::setParentPrincipal(MainWindow *parent)
{
    parentWindow = parent;
}

void agenda_ui::update_table()
{
    this->on_pushButton_showSelect_clicked();
}
